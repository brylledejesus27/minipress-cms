...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_FILES["media_file"])) {
    ...
    if (!in_array($fileType, $allowedTypes)) {
        ...
        if (move_uploaded_file($fileTmp, $filePath)) {
            ...
            $stmt = $conn->prepare("INSERT INTO media (filename, original_name, file_type, file_size, uploaded_by) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssii", $newFileName, $originalName, $fileType, $fileSize, $userId);
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    if ($fileResult && $fileResult->num_rows > 0) {
        ...
        $realPath = "../assets/uploads/" . $file['filename'];
...
$media = $conn->query("SELECT * FROM media ORDER BY created_at DESC");
$totalMedia = 0;
...
<style>
.media-upload-card { margin-bottom: 22px; }
.media-upload-form {
    display: flex; gap: 12px;
    align-items: center; flex-wrap: wrap;
}
.media-upload-form input[type="file"] {
    flex: 1; min-width: 240px; padding: 13px;
...
.media-meta { font-size: 12px; color: #6b7280; margin-top: 4px; }
.media-actions { display: flex; gap: 8px; margin-top: 12px; }
.media-actions a { flex: 1; text-align: center; text-decoration: none; }
.view-btn {
...
.empty-media { text-align: center; padding: 40px; color: #6b7280; }
</style>
</head>
                        ...
                        <div class="media-card">
                            <div class="media-preview">
                                <?php if (str_starts_with($row['file_type'], 'image/')): ?>
                                    <img src="../assets/uploads/<?php echo htmlspecialchars($row['filename']); ?>" alt="Media">
                                <?php else: ?>
                            ...
                            <div class="media-meta">
                                <?php echo round($row['file_size'] / 1024, 1); ?> KB &bull;
                                <?php echo date("M d, Y", strtotime($row['created_at'])); ?>
                            </div>
                            <div class="media-actions">
                                <a class="view-btn" href="../assets/uploads/<?php echo htmlspecialchars($row['filename']); ?>" target="_blank">View</a>
